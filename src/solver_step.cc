...
int main()
{
    ...
    array<size_t, 3> n     {512, 128, 64};          //  real space dims
    ...
    double b_z   = .1,   //  heat conductivity along z (related to but not independent of a[2])
           ...
           t     = 7,    //  evaluation time
           m     = 40;   //  number of positive frequencies used to approximate the step
    ...
    complex<double> *s = new complex<double>[n[0] * n[1] * (2 * m + 1)];
    ...
    complex<double> *m_T_dft = new complex<double>[n[0] * n[1] * (2 * m + 1)],
                    *m_T     = new complex<double>[n[0] * n[1] * (2 * m + 1)],
