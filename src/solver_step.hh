...
void eval_sigma_1(std::complex<double> *const s, const std::array<std::size_t, 3> n,
                  const double *const k_x, const double *const k_y, const std::array<double, 3> a, const double q_f)
{
    using namespace std::complex_literals;
    for (std::size_t j = 0; j < n[1]; j++)
        for (std::size_t i = 0; i < n[0]; i++)
...
void eval_T(std::complex<double> *const T, const std::complex<double> *const m_T,
            const std::array<std::size_t, 3> n, const double m, const double q_p, const double t)
{
    using namespace std::complex_literals;
    for (std::size_t i = 0; i < n[0] * n[1]; i++)
    {   T[i] = .5 * m_T[i];
        for (std::size_t j = 1; j < 2 * m + 1; j++)
        {   double k = 2 * (j - m) - 1;
